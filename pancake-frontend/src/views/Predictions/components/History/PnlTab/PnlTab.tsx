...
import { formatEth, getMultiplier, getNetPayout } from '../helpers'
...
const PnlTab: React.FC<PnlTabProps> = ({ hasBetHistory, bets }) => {
  ...
  const avgEthWonPerRound = netResultAmount / summary.entered.rounds
  const avgEthWonIsPositive = avgEthWonPerRound > 0
      ...
      <Box pl="8px">
        ...
        {hasBestRound && (
          <>
            ...
            <Text small color="textSubtle">
              {`~$${formatEth(ethBusdPrice.times(summary.won.bestRound.payout).toNumber())}`}
            </Text>
          </>
        )}
        ...
        <SummaryRow type="won" summary={summary} ethBusdPrice={ethBusdPrice} />
        <SummaryRow type="lost" summary={summary} ethBusdPrice={ethBusdPrice} />
        <SummaryRow type="entered" summary={summary} ethBusdPrice={ethBusdPrice} />

        <Flex justifyContent="center" mt="24px">
          <Link href={`${getBlockExplorerLink(account, 'address')}#internaltx`} mb="16px" external>
