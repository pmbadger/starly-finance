...
const getButtonProps = (
  ...
  const hasSufficientBalance = () => {
    if (value.gt(0)) {
      return value.lte(ethBalance)
    }
    return ethBalance.gt(0)
  }
...
const SetPositionCard: React.FC<SetPositionCardProps> = ({ position, togglePosition, onBack, onSuccess }) => {
  ...
  // Convert eth balance to ethers.BigNumber
  const ethBalanceAsBn = useMemo(() => {
    return ethers.BigNumber.from(ethBalance.toString())
  }, [ethBalance])
  const maxBalance = useMemo(() => {
    return ethBalanceAsBn.gt(dust) ? ethBalanceAsBn.sub(dust) : dust
  }, [ethBalanceAsBn])
  const balanceDisplay = formatBigNumber(ethBalanceAsBn)
