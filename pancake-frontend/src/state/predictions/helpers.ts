...
export const transformTotalWonResponse = (
  ...
  const totalETH = marketResponse.totalETH ? parseFloat(marketResponse.totalETH) : 0
  const totalETHTreasury = marketResponse.totalETHTreasury ? parseFloat(marketResponse.totalETHTreasury) : 0

  return Math.max(totalETH - (totalETHTreasury + houseRounds), 0)
}
